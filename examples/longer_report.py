...
pb.print_html("<br>")
pb.print_div("However, we can use special function to write text into a &lt;br&gt; element.")
pb.print_html("<br>")
pb.print_div("It is possible to pass several values and optionally a separator.",
...
pb.print_div(f"We can also add a link to {pb.link('Python documentation', 'https://www.python.org/doc/')} "
             f"if necessary. Talking about links, note that each heading has a clickable anchor.")
pb.print_html("<br>")
pb.print_div(f"We already used {pb.code('code')} function for inline formatting. There is also {pb.code('print_code')} "
...
pb.print_div(f"Before going further, let's return to the very first function we used: {pb.code('set_title')}. "
...
pb.print_div(f"It is also possible to create references to tables and figures. "
             f"For example Table {table_ref} shows sortable columns and Fig. {img_reference} displays a scatterplot.")
pb.print_table(df, caption='A sortable table with a reference', reference=table_ref, sortable=True)

pb.print_table(df, caption='A table sorted by y column', sorting_definition=('y', 'asc'))
fig, ax = plt.subplots()
...
pb.print_div(f"Note that you can use the references in your text multiple times, "
