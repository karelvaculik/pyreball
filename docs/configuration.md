...
* `css.template` - Contains CSS definitions that are inserted into the final HTML file.
* `html.template` - Contains the template of the HTML file. It contains three placeholders:
    * `<!--PYREBALL_PAGE_TITLE-->` - Here goes the title set
      ...
      function
      or Table-of-Contents title. If the title is not set, nor Table-of-Contents is created, the file stem is used as
      the value of `<title>` element.
    ...
    * `<!--PYREBALL_INLINE_HIGHLIGHT_SCRIPT-->` - Placeholder for inline JavaScript code that highlights syntax of
      inline code snippets. Pyreball decides dynamically whether the script is included or not.
...
Each key-value pair represents a library with the relevant links. For example, if your Pyreball report creates
